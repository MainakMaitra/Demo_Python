def analyze_transcript_length_buckets_pre_post(df_main, problematic_rules=None):
    ...
    # 5. PROBLEMATIC RULES SPECIFIC ANALYSIS (if provided)
    if problematic_rules is not None and len(problematic_rules) > 0:
        print("\n5. PROBLEMATIC RULES SPECIFIC ANALYSIS")
        print("-" * 50)
        
        # Analyze top 5 problematic rules
        top_problematic = problematic_rules.head(5)
        ...
        for idx, rule in top_problematic.iterrows():
            ...
            print(f"\nRule: {event} | {query} (Negations: {negation_count})")
            ...
            if len(rule_data) > 0:
                rule_length_analysis = rule_data.groupby(['Category_Focus', 'Period']).agg({
                    'Primary Marker': ['count', lambda x: (x == 'TP').mean()]
                }).reset_index()
                
                rule_length_analysis.columns = ['Category_Focus', 'Period', 'Count', 'Precision']
                
                if len(rule_length_analysis) > 0:
                    rule_pivot = rule_length_analysis.pivot_table(
                        index='Category_Focus',
                        columns='Period',
                        values='Precision',
                        fill_value=0
